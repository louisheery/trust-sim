...
public class WangTrustAgent implements Agent {
  ...
  public WangTrustAgent(Graph graph, int idInput, List<Double> dimensions, List<Double> agentPersonalityDimensions) {
    this.graph = graph;
    this.id = idInput;
...
//    }
  ...
  public void updateTrustValues(Agent otherAgent) {
    ...
    List<List<Agent>> agentToOtherAgentPaths = graph.getAllRoutesBetweenAgents(this, otherAgent);
    ...
    double indirectTrust = graph.getIndirectTrustVector(otherAgent).getAverageTrustVectorValue();
    ...
    TrustVectorList trustVector = graph.getDirectTrustVector(this, otherAgent);
  ...
  public Double requestTrustScoreInAnotherAgent(Agent otherAgent) {
    ...
    if (graph.hasDirectConnection(this, otherAgent)) {
      return graph.getDirectTrustVector(this, otherAgent).getTrustVectorValue(0);
    } else {
      return null;
    }
